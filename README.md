...
Official prebuilt package (embedded CurseForge key — the full experience), x86_64 (amd64) only. aarch64 users: build from source below (Qt has no official 6.11 Linux ARM64 packages, so no prebuilt for now):
